...
class DocumentRecord {
  ...
  _dependenciesDeferred: Deferred<Set<string>>;
  ...
  constructor(urlOrFrom: string|DocumentRecord) {
    if (typeof urlOrFrom === 'string') {
      ...
      this._dependenciesDeferred = new Deferred<Set<string>>();
    } else {
      ...
      this._dependenciesDeferred = new Deferred<Set<string>>();
      if (from._dependenciesDeferred.resolved) {
        this._dependenciesDeferred.resolve(this.dependencies);
      } else if (from._dependenciesDeferred.rejected) {
        this._dependenciesDeferred.reject(from._dependenciesDeferred.error);
      }
    }
  }

  get dependenciesKnown(): Promise<Set<string>> {
    return this._dependenciesDeferred.promise;
  }
}
...
export class DependencyGraph {
  ...
  addDocument(url: string, dependencies: Iterable<string>) {
    ...
    try {
      record._dependenciesDeferred.resolve(record.dependencies);
    } catch (e) {
  ...
  rejectDocument(url: string, error: Error) {
    this._getRecordFor(url)._dependenciesDeferred.reject(error);
  }
  ...
  private async _whenReady(key: string, visited: Set<string>) {
    ...
    visited.add(key);
    const deps = await this._getRecordFor(key).dependenciesKnown;
